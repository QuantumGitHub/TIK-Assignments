...
void list_push(struct node *head, struct node *node)
{
	node->next = head->next;
	head->next->prev = node;
	head->next = node;
...
void list_pop(struct node *head, struct node *node)
{
	node->next->prev = head;
	head->next = node->next;
	node->next = node;
...
/*
 * If there are 3 elements on the list, and i is 3, you'll get the first (i.e.,
 * wraps)
 */
struct node *list_get(struct node *head, size_t i)
{
	if (!head) return NULL;

	if (!i) return head; /* Done */

	return list_get(head->next, i--);
}
