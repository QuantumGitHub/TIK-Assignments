...
static const struct test ts[] = {
	...
	[access_demand_mega_normal_pages] = { "access_demand_mega_normal_pages",
					      1 },
	[access_demand_nomega_overlap] = { "access_demand_nomega_overlap", 1 },
...
int pt_test(void)
{
	...
	if (!(ptm.vma = pt_alloc_vma(kvmas_head))) goto should_not_happen;
	...
	for (tm.t = 0; tm.t < __success; tm.t++) {
		...
		case access_demand_several_pages:
			...
			sp_read(sp);
			sp = sp - PAGE_SIZE;
			tf_user = (struct trap_frame *)(sp - (264));
			csrw(sscratch, sp);
		...
		map_mega_normal_pages:
			...
			if (tm.t == access_demand_nomega_overlap)
				goto access_demand_nomega_overlap;
		...
		case access_demand_nomega_overlap:
			goto map_mega_normal_pages;
		access_demand_nomega_overlap:
			update_args();
			printf("%s: mapping them again, but this time the page faults occur in a different order...\n",
			       args);

			*(ptm.virt + 3 * PAGE_SIZE) = 0xb;

			if (pt_get_pte(&ksatp, ptm.vpn + 3, ptm.ptes, NULL) !=
			    3)
				goto map_failure;
		...
		case map_several_vmas:
			...
			if (!(another_vma = pt_alloc_vma(kvmas_head)))
